//
// Created by Eric Irrgang on 4/8/18.
//
/*! \file
 * \brief Test the ability of restraints to provide and register callbacks.
 *
 * Using the Restraint<PotentialT> template, check a potential that provides no callback,
 * one that provides a callback that does not need EnsembleResources, and one that provides
 * a callback that takes EnsembleResources. Get an IRestraintPotential pointer for each and
 * call IRestraintPotential::update().
 */
...
class RestraintWithoutCallback
{
    public:
        struct input_param_type {};

        gmx::PotentialPointData calculate(gmx::Vector,
                                                            gmx::Vector,
                                                            double) { return {}; };
};

TEST(RestraintCallbacks, noCallback)
{
    auto resources = plugin::EnsembleResources::Builder().build();
    const RestraintWithoutCallback::input_param_type params{};
    auto restraint = std::make_shared<::plugin::Restraint<RestraintWithoutCallback>>(
        std::vector<unsigned long>(0,0),
        params,
        resources);
    ASSERT_NO_THROW(restraint->update(gmx::Vector(), gmx::Vector(), double()));
}
//
